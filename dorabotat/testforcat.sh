...
diff_files() {
    ...
    # Добавляем оба файла в вывод
    "$my_program" $flags "$filename1" "$filename2" >> "$my_testfile"
    "$program" $flags "$filename1" "$filename2" >> "$testfileorig"
...
echo  "${COLOR_YELLOW}ОДИНОЧНЫЕ ДЛИННЫЕ ФЛАГИ:${COLOR_RESET}"
